...
class ServiciosPuntoVentaController extends Controller {
    ...
    public function actionCrear() {
        if (Yii::app()->request->isPostRequest) {
            ...
            if (!Yii::app()->user->checkAccess('PuntoDeVenta_ServiciosPuntoVenta_crear')) {
            ...
            if ($render) {
                ...
                Yii::app()->end();
            } else if (isset($_POST['ServiciosPuntoVenta'])) {
                $servicios = $_POST['ServiciosPuntoVenta'];
                $models = array();

                foreach ($servicios as $indice => $servicio) {
                    $model = new ServiciosPuntoVenta;
                    $model->attributes = $servicio;
    ...
    /*public function actionEliminar() {
        if (Yii::app()->request->isPostRequest) {
            ...
            if (!Yii::app()->user->checkAccess('PuntoDeVenta_ServiciosPuntoVenta_eliminar')) {
                echo CJSON::encode(array('result' => 'error', 'response' => 'Error: 101 => ' . Yii::app()->params['accessError']));
                Yii::app()->end();
            }
            ...
            $servicio = Yii::app()->getRequest()->getPost('servicio', null);

            if ($servicio === null || empty($servicio)) {
                echo CJSON::encode(array('result' => 'error', 'response' => 'No se detecta servicio'));
                Yii::app()->end();
            }

            try {
                $model = $this->loadModel($puntoventa, $servicio);
     ...
     */
    public function loadModel($idpuntoventa, $idtiposervicio) {
        $model = ServiciosPuntoVenta::model()->findByPk(array('IDPuntoDeVenta' => $idpuntoventa, 'IDTipoServicio' => $idtiposervicio));
